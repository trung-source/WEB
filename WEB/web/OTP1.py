...
def TOTP(K,tm,Digit = 8):                                                   # TOTP theo bài báo rfc6238
...
def validate(Key,counter):                                                                  # Validate  
    ...
    D1 = TOTP(Key,counter)                                                # Do độ trễ của mạng thì quá trình kiểm tra rất có thể sẽ bị tính toán nhanh hơn 1 counter so với bên user
    D2 = TOTP(Key,counter-1)                                              # Để đảm bảo cho việc xác thực thì phải thêm cả trường hợp timestamp trước đó
