...
def init_secret(p = 509, q = 607, secret = 0):
    ...
    if secret == 0:
        K = int(RSACS.genkey(p,q),base=2)                                           # Khóa secret khởi tạo sẽ có chiều dài từ 20 - 64 bytes
        # K2 = hex(K)
        # K3 = hex(K)
        ...
        K1 = OTP.HMAC1(K,counter)                                                   # Khóa secret để share với user (counter = 0)
...
def sharekey():                                                                     # Khoa chia se
    file = open('Python\WEB\OTPfold\secret.txt','r+')                                   # 
    secret = file.read()
...
def reset_key():                                                                     # Hàm encode khóa
    file = open('Python\WEB\OTPfold\secret.txt','w')  
    secret = init_secret()
